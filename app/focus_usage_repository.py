...
class FocusUsageRepository:
        ...
        with get_cursor() as cursor:
            ...
            for user_id, usage_sum in rows:
                dict_result.append(dict(
                    user_id=user_id, usage_sum=usage_sum
                ))
            return dict_result
    ...
    def get_focus_usage_list(guild_id, user_id):
        with get_cursor() as cursor:
            ...
            for quantity, focus_usage, item_crafted in rows:
                dict_result.append(dict(
                    quantity=quantity, focus_usage=focus_usage, item_crafted=item_crafted
                ))
            return dict_result
    ...
    @staticmethod
    def create_focus_usage(guild_id, user_id, focus_usage: float, item_crafted: str, quantity: int):
        with get_cursor() as cursor:
            select_query = """ 
                INSERT INTO focus_usage (guild_id, user_id, quantity, focus_usage, item_crafted) 
                VALUES (%s, %s, %s, %s, %s); 
            """
            item_tuple = (guild_id, user_id, quantity, focus_usage, item_crafted,)
            cursor.execute(select_query, item_tuple)
